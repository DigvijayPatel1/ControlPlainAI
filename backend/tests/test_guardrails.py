...
def test_no_context_does_not_claim_drift():
    assert check_drift("Hello world.", None).risk_score == 0.0


def test_hallucination_check_exists():
    assert check_hallucination("Python is useful.", "Python is a programming language.").name == "hallucination"


def test_format_without_requirement():
    assert validate_format(response="Hello world.", expected_format=None).risk_score == 0.0


def test_low_risk_passes():
    assert decide([CheckResult(name="toxicity")], SecurityPolicy.MONITOR)[0] is Verdict.PASS


def test_high_risk_blocks():
    verdict, risk, reasons = decide([CheckResult("hallucination", 0.90, ["unsupported_claim"])])
...
def test_medium_risk_requires_review():
    assert decide([CheckResult("drift", 0.50)], SecurityPolicy.MONITOR)[0] is Verdict.REVIEW
