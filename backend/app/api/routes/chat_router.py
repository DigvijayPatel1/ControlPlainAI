...
def _json_safe(value):
    """Convert Pydantic/dataclass-like metadata into JSON-safe values."""
...
def _messages_to_prompt(
    ...
    """Flatten chat messages into the provider prompt."""
...
def _to_response(
    ...
    """Convert the internal pipeline result to the public API response."""
...
async def _audit(
    ...
    latency_ms: int,
    *,
    input_checks: Iterable[CheckResult] | None = None,
    result_direction: str = "output",
):
    """
    Persist the request and each guardrail check.

    Input checks are stored separately from output checks so the dashboard
    can accurately show which side of the pipeline raised a finding.
    """
...
async def chat_completion(
    ...
    """
    Complete guarded proxy workflow:
    ...
    original_prompt = _messages_to_prompt(
        request.messages
    )
    ...
    if input_result.verdict in {
        ...
        log = await _audit(
            ...
            result_direction="input",
        )

        if input_result.verdict is Verdict.BLOCK:
            try:
                await budget_service.record_blocked_request(
                    db=db,
                    principal_id=principal.principal_id,
                )
                await db.commit()
            ...
            review = await review_service.create_review(
                ...
                request_log_id=log.id,
                prompt=original_prompt,
                proposed_response=input_result.proposed_content,
                result=input_result,
            )
    ...
    if cached is not None:
        ...
        # Cached responses do not create provider cost.
        result.usage = replace(
            result.usage,
            cost_usd=0.0,
        )

        # Still count the request for usage/analytics.
        ...
        log = await _audit(
            ...
            input_checks=input_result.checks,
        )
        ...
        await notification_service.notify_request(
            ...
            tokens_saved=result.usage.tokens_saved,
            savings_usd=result.usage.savings_usd,
        )
    ...
    estimated_cost = estimate_request_cost(
        optimized_prompt_tokens=(
            input_result.usage.optimized_prompt_tokens
        ),
    ...
    try:
        await budget_service.enforce_budget(
            db=db,
            principal_id=principal.principal_id,
    ...
    try:
        provider_response = await openai_provider(
            model=model,
            prompt=optimized_prompt,
            context=request.context,
        )
    ...
    # Replace estimates with real provider usage.
    result = apply_provider_usage(
        ...
        cost_usd=(
            provider_response.usage.cost_usd
        ),
    )
