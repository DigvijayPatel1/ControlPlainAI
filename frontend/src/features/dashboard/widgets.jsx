...
export function Metric({ label, value, tone = '' }) {
    return (<div className="rounded-xl border border-line bg-card p-4">
...
export function Panel({ title, children }) {
    return (<section className="mb-6 rounded-2xl border border-line bg-card p-5">
...
export function Empty({ text }) {
    return (<div className="rounded-xl border border-dashed border-line py-10 text-center text-sm text-muted">
            {text}
        </div>);
}
...
export function RequestTable({ rows }) {
    if (!rows.length)
        return <Empty text="No request telemetry yet."/>;
    return (<div className="overflow-x-auto rounded-xl border border-line">
            <table className="w-full border-collapse text-sm">
                ...
                <tbody>
                    {rows.map((row) => (<tr key={row.id} className="border-b border-line last:border-b-0 hover:bg-paper/60">
                            <td className="px-4 py-3 text-ink">{new Date(row.timestamp).toLocaleString()}</td>
                            <td className="px-4 py-3">
                                <span className={`rounded-full px-2 py-0.5 font-mono text-xs font-semibold uppercase ${VERDICT_STYLES[row.verdict.toLowerCase()] ?? 'bg-line text-ink'}`}>
                                    {row.verdict}
                                </span>
                            </td>
