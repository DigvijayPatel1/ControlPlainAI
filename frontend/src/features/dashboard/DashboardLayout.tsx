...
const navLinkClasses = ({ isActive }: { isActive: boolean }) =>
    `block rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
        isActive ? 'bg-brand-green/20 text-mint' : 'text-white/70 hover:bg-white/5 hover:text-white'
    }`
            ...
            <aside className="flex w-64 shrink-0 flex-col bg-sidebar px-4 py-6 text-white">
                ...
                <div className="mt-6 border-t border-white/10 pt-4">
                    ...
                    <button
                        ...
                        className="w-full rounded-lg border border-white/15 py-2 text-sm font-medium text-white/80 transition-colors hover:bg-white/10 hover:text-white"
                    >
                        Sign out
                    </button>
                </div>
