...
export default function ApiKeyCard() {
    ...
    return (
        <div className="rounded-2xl border border-brand-amber/30 bg-amber-50/60 p-5">
            ...
            <div className="flex flex-wrap items-center gap-3">
                <button
                    ...
                    className="rounded-lg border border-line px-4 py-2 text-sm font-medium text-ink transition-colors hover:bg-white"
                >
                ...
                {isExtensionConfigured() ? (
                    <button
                        ...
                        className="rounded-lg bg-brand-green px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-brand-green/90 disabled:opacity-60"
                    >
                        {connectState === 'connected' ? 'Connected ✓' : connectState === 'connecting' ? 'Connecting…' : 'Connect extension'}
                    </button>
                ) : (
                    <span className="text-sm text-muted">Set VITE_EXTENSION_ID to enable one-click connect.</span>
                )}

                <button
                    type="button"
                    onClick={() => dispatch(apiKeyAcknowledged())}
                    className="rounded-lg px-4 py-2 text-sm font-medium text-muted transition-colors hover:bg-white/60"
                >
            ...
            {connectState === 'failed' && (
                <p className="mt-3 rounded-lg border border-brand-red/30 bg-red-50 px-3 py-2 text-sm text-brand-red">
